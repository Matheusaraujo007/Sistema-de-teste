...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      // Calcula valor total se não enviado
      let total = valorTotal;
      if ((!total || total === 0) && itens) {
        total = itens.reduce((acc, i) => acc + (parseFloat(i.quantidade) || 0) * (parseFloat(i.valorUnit) || 0), 0);
      }
      ...
      if (itens) {
        // Calcula valor total automaticamente a partir dos itens
        const total = itens.reduce((acc, i) => acc + (parseFloat(i.quantidade) || 0) * (parseFloat(i.valorUnit) || 0), 0);
